...
export const LoginUserDB = (user)=>{
    return (dispatch)=>{
        axios.post("http://localhost:9000/user/api/login",//uri or end point of singninup api
...
export const SaveHobbiesToDBUsingFetch = (user)=>{
    ...
    console.log(user)
    return (dispatch)=>{
        window.fetch("http://localhost:9000/user/api/addhobby",
            {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
...
export const getHobbiesFromDB = (userName) => {
    return (dispatch) => {
        axios.get("http://localhost:9000/student/api/getHobbies/${userName}")
            .then((response) => {
